...
func InitApp() *App {
	...
	uuidFn := ioc.InitUuidFn()
	rankingJobSet := ioc.InitRankingJobAndReportLoadJob(rankingService, loadService, rlockClient, cmdable, uuidFn, loggerV1)
	cron := ioc.InitJobs(loggerV1, rankingJobSet)
	app := &App{
		web:       engine,
		consumers: v2,
		cron:      cron,
